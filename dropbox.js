...
function implictGrant(res, userID){
	var para = 'client_id='+ client_id +'&response_type=token&redirect_uri='+ encode.uriEncode(redirect_uri) +'&state='+ userID;
...
function doImplictGrantAuth(res){
	var ip = Net.getLocalIP("en0");

	implictGrant(res, ip);
}

function startImplictGrantServ(app){
	app.get(serv_uri+"/implict", function(req, res){
		//console.log("===== do implict Grant Auth =========");
		doImplictGrantAuth(res);
	});
}

exports.startCodeGrantServ = startCodeGrantServ;
exports.startImplictGrantServ = startImplictGrantServ;
